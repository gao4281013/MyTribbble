...
class LisenseActivity : AppCompatActivity() {

  override fun onCreate(savedInstanceState: Bundle?) {
    super.onCreate(savedInstanceState)
    setContentView(R.layout.activity_lisense)
    showLisense()
  }

  private fun showLisense() {
    val notices = Notices()
    ...
    val lisenseDialogFragment = LicensesDialogFragment.Builder(this).setNotices(notices)
        .setShowFullLicenseText(false)
        .setIncludeOwnLicense(true)
        .setUseAppCompat(true)
        .build()

    lisenseDialogFragment.show(supportFragmentManager,null)
    lisenseDialogFragment.setOnDismissListener { finish() }
  }
}
